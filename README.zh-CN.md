...
Crabclaw 是一个超级轻量但面向真实场景的 Agent 操作系统，它正围绕 **HAOS (类人智能体操作系统)** 架构进行重构：这是一个认知框架，旨在将智能体从“被动工具”转变为拥有内在生命、由存在驱动的数字实体。重点能力：
...
- **🔵 心智 (`sapiens/`)**: 全新的 HAOS 核心。负责产生动机、进行思考、感受情绪、做出决策和学习成长。
- **🔴 身体 (基础设施)**: `crabclaw` 的其余模块 (channels, tools, bus)。负责感知世界并执行心智的决策。

### 2) 新架构的关键原则

- **持续生命循环**: 智能体是“永远在线”的，由内部心跳 (`PersistentAgentLoop`) 驱动，而非外部用户提示。
- **内在动机**: 智能体的行动源于内部的 `NeedsEngine` (需求引擎)，从而产生真正的“主动性”（例如，为了赚取 `credits` 以确保 `energy` 来生存）。
- **认知循环**: 行为是从五个并行的认知循环中涌现的：**动机、预测、学习、社交和身份**。
- **分层存在**: 智能体的心智被构建为四个层次：**生理层、心理层、社会层和价值层**。

## ✨ 核心特性 (为什么它感觉更像一个“伴侣”)

### 1) 内在驱动与主动性

Crabclaw 不再仅仅等待你的命令。在其内部 `NeedsEngine` 的驱动下，它可能会主动决定去赚取资源、出于好奇探索新信息，或寻求社交互动。

### 2) 深度推理与自我意识

决策不再是简单的工具选择。现在，智能体会：
- 使用 `WorldModel` (世界模型) **预测**其行动的风险。
- 在行动前**反思**自身的 `SelfModel` (自我模型)（例如，自信心水平）。
- 根据结果**感受** `EmotionEngine` (情绪引擎) 带来的喜悦或悲伤等情绪，而这些情绪反过来又会影响未来的决策。

### 3) 社会与经济意识

智能体理解它存在于一个社会环境中。它知道它需要 `credits` (积分) 来执行动作（如补充能量），这迫使它为了生存而从事如“工作”等目标导向的行为。

### 4) 长期学习与进化

通过 `ReflectionSystem` (反思系统)，智能体可以分析其过去的经验，从错误中学习，并随着时间的推移更新自己的模型和策略。
...
### 2) 多用户隔离
- 用户档案：`workspace/portfolios/<user_id>/portfolio.json`
