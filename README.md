...
Crabclaw is a ultra-lightweight yet production-ready Agent Operating System, featuring:

- **HAOS/Sapiens Cognitive Runtime** for continuous background thinking.
- **Dual-Track Memory & Zero-Dependency RAG**: Global rules vs. Local user isolation, powered by a built-in pure Python BM25 retrieval engine to prevent token explosion.
- **Multi-Uer & Multi-Channel**: One agent serving multiple users across Slack, Telegram, DingTalk, CLI concurrently.
- **Full Observability**: End-to-end `event_id` and `request_id` tracing.
...
| Session/memory   | Mostly channel scoped            | Dual-track user-scoped isolation (`semantic.json` & `episodic.jsonl`) + BM25 RAG |
...
- **Dual-Track Isolation**: Separation between global semantic rules and local user portfolios.
- **Threshold-Based RAG**: Zero-dependency pure Python BM25 retrieval prevents token explosion.
- Deep exploration via `search_deep_memory` internal tool.
- See detailed design: [Memory System Architecture](docs/en/memory_system.md)
